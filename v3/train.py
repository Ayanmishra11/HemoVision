...
from v3.config import TRAIN_CFG, TASK_NAMES, TASK_TIERS
from v3.dataset import HemoWaveformDataset, SubjectCameraSampler
from v3.model import HemoVisionBoundedTCN_V3
from v3.loss import HemoMultiTaskLoss_V3, fft_hr_bpm, batch_pearson_correlation
...
def training_epoch(model, criterion, loader, optimizer, scheduler, scaler, biomarker_mean, biomarker_std) -> Dict[str, float]:
    model.train()
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        camera_onehot = batch["camera_onehot"].to(DEVICE, non_blocking=True)

        biomarker_target = standardize_biomarkers(biomarkers_raw, biomarker_mask, biomarker_mean, biomarker_std)

        with torch.cuda.amp.autocast(enabled=USE_AMP):
            prediction = model(x, x_raw, camera_onehot)
            losses = criterion(prediction, bvp_target, biomarker_target, biomarker_mask, fps)
            
            # Gradient accumulation division
            loss = losses["total"] / TRAIN_CFG.grad_accumulation_steps

        scaler.scale(loss).backward()

        if batch_index % TRAIN_CFG.grad_accumulation_steps == 0 or batch_index == total_batches:
        ...
        if batch_index % PROGRESS_EVERY_N_BATCHES == 0 or batch_index == total_batches:
            print(f"  Train batch {batch_index:04d}/{total_batches:04d} | loss={losses['total'].item():.5f} | r={losses['batch_pearson_r'].item():.4f} | LR={scheduler.get_last_lr()[0]:.2e}", flush=True)
...
def main():
    ...
    # Use SubjectCameraSampler for training to avoid label duplication
    train_sampler = SubjectCameraSampler(train_dataset, shuffle=True)
    train_loader = DataLoader(train_dataset, batch_size=TRAIN_CFG.batch_size, sampler=train_sampler, num_workers=TRAIN_CFG.num_workers, pin_memory=USE_AMP, drop_last=True)
    
    # Validation evaluates all cameras as usual
    validation_loader = DataLoader(validation_dataset, batch_size=TRAIN_CFG.batch_size, shuffle=False, num_workers=TRAIN_CFG.num_workers, pin_memory=USE_AMP, drop_last=False)
    ...
    model = HemoVisionBoundedTCN_V3().to(DEVICE)
    criterion = HemoMultiTaskLoss_V3().to(DEVICE)
    optimizer = AdamW(model.parameters(), lr=TRAIN_CFG.learning_rate, weight_decay=TRAIN_CFG.weight_decay)
    scaler = torch.cuda.amp.GradScaler(enabled=USE_AMP)

    # Cosine Annealing LR Scheduler
    total_steps = len(train_loader) // TRAIN_CFG.grad_accumulation_steps * TRAIN_CFG.epochs
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10 * (len(train_loader) // TRAIN_CFG.grad_accumulation_steps), T_mult=2)
    ...
    for epoch in range(1, TRAIN_CFG.epochs + 1):
        ...
        train_metrics = training_epoch(model, criterion, train_loader, optimizer, scheduler, scaler, BIOMARKER_MEAN, BIOMARKER_STD)
