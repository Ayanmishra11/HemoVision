...
RUN_TRAINING = True  # Set True only after confirming the self-tests and dataset summary above.
...
# %%
# ==========================================
# BEST-CHECKPOINT HELD-OUT EVALUATION
# ==========================================
...
best_model = HemoVisionV3().to(DEVICE)
best_model.load_state_dict(checkpoint["model_state_dict"])
best_model.eval()

best_mean = checkpoint["biomarker_mean"].to(DEVICE)
best_std = checkpoint["biomarker_std"].to(DEVICE)

print(
    f"Loaded best checkpoint: epoch={checkpoint['epoch']} | "
    f"best validation score={checkpoint.get('best_validation_score', checkpoint.get('best_val_loss'))}"
)

best_validation_results = evaluate(
    model=best_model,
    loader=val_loader,
    mean=best_mean,
    std=best_std,
...
print(json.dumps(best_validation_results, indent=2))

print("\nBEST-CHECKPOINT HELD-OUT TEST:")
print(json.dumps(best_test_results, indent=2))

with open(
    CFG.checkpoint_path.with_name("hemovision_v3_best_heldout_results.json"),
    "w",
    encoding="utf-8",
) as handle:
    json.dump(
        {
            ...
            "validation": best_validation_results,
            "heldout_test": best_test_results,
        },
        handle,
        indent=2,
        allow_nan=True,
